...
export async function createTenant(
  ...
  const { data: tenant, error: tenantError } = await admin
    .from("tenants")
    // couleur_primaire optionnelle (meme logique que updateBranding dans
    // app/admin/etablissement/actions.ts : simple verite truthy, pas de
    // validation de format, le <input type="color"> garantit deja un hex
    // valide cote navigateur) - permet de choisir la couleur de marque des
    // la creation plutot que seulement apres coup via la page Branding.
    .insert(couleurPrimaire ? { nom, slug, couleur_primaire: couleurPrimaire } : { nom, slug })
    .select("id")
    .single();
